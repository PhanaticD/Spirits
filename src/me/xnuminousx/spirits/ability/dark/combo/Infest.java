...
public class Infest extends DarkAbility implements AddonAbility, ComboAbility {
	...
	public void spawnCircle() {
		...
		for (int i = 0; i < 6; i++) {
			...
			location2.add(x, 0, z);
			ParticleEffect.SMOKE_NORMAL.display(location2, 1);
			location2.subtract(x, 0, z);
			
			location3.add(x2, 0, z2);
			ParticleEffect.SMOKE_NORMAL.display(location3, 1);
			location3.subtract(x2, 0, z2);
		}
		
		ParticleEffect.DRAGON_BREATH.display(location, 1, radius / 2.0, 0.4F, radius / 2.0, 0.01F);
	}
			...
			if (entity instanceof Player) {
				...
				if (bEntity.hasElement(Element.getElement("DarkSpirit"))) {
					if (healDarkSpirits) {
						...
						ParticleEffect.HEART.display(entity.getLocation().add(0, 2, 0), 1);
					}
				} else {
				...
				ParticleEffect.VILLAGER_ANGRY.display(entity.getLocation().add(0, 1, 0), 1);
